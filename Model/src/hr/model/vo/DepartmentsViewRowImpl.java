...
import hr.model.eo.DepartmentsImpl;


import hr.model.vo.common.DepartmentsViewRow;
...
//        while(rows.hasNext()) {
//            EmployeesViewRowImpl row = (EmployeesViewRowImpl) rows.next();
//            row.setSalary( row.getSalary().add(new BigDecimal(amount)));
//        }
       RowIterator rows =    this.getEmployeesView();
       while(rows.hasNext()) {
           Row r =  rows.next();
           r.setAttribute("Salary",  ((Number)r.getAttribute("Salary")).intValue() + amount );
       }
